...
namespace sponza
{
	void InitialiseLights(sponza::PointLight lights[4])
	{
		...
		lights[0].ambient = glm::vec4(0.0f, 1.0f, 0.0f, 1.0);

		lights[1].position = glm::vec4(-617.901, 188.135, 143.653, 0.0);
		lights[1].ambient = glm::vec4(1.0f, 0.0f, 0.0f, 1.0);

		lights[2].position = glm::vec4(-36.8992,769.785,34.8048, 0.0);
		lights[2].ambient = glm::vec4(0.0f, 0.0f, 1.0f, 1.0);

		lights[3].position = glm::vec4(1034.76,148.346,-33.2571, 0.0);
		lights[3].ambient = glm::vec4(0.5f, 0.3f, 0.2f, 1.0);
